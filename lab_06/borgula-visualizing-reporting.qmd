...
```{r, message=FALSE}
SF <- read_csv("/Users/oliviaborgula/Documents/GitHub/olivia_borgula_jour472/lab_06/SF_311.csv")
```
...
```{r}
#create dataframe that filters for arrests and totals them by year
arrests <- SF %>% 
...
arrests_pivot <- dispo %>% 
  pivot_wider(names_from = "disposition", values_from = "n") %>% 
  as.data.frame()
#2018 had the most, with 159 arrests in the year
```


Then create a bar chart that colors the bars by arrest total. Include a headline and caption
```{r}
#create a bar chart that colors the bars by arrest total
arrests %>%
  ggplot(aes(x=year, y=n, fill = n, weight=n)) +
  theme_minimal() +
  geom_col(position = "dodge")+
    labs(
    fill = "Frequency",
    title="San Francisco Arrests from 2016 - 2019",
    x = "Year",
    y = "Number of Arrests",
    caption = "Source: SF PD Service Call Data, 2016-2019")
...
```{r}
#build a table counting dispositions
all_dispo <- SF %>%
  count(disposition)%>%
  slice_max(disposition, n = 12)%>%
  arrange(desc(n))
View(all_dispo)
#most common disposition is HAN, or handled
```

Create a bar chart of all_dispo and include a headline and caption

```{r}
#create bar chart of all_dispo w hl and caption
all_dispo %>% 
  ggplot(aes(x=disposition, y=n, fill=n, weight=n)) +
  coord_flip()+
  theme_minimal() +
  geom_col(position = "dodge")+
    labs(
    fill = "Frequency",
    title="San Francisco Dispositions from 2016 to 2019",
    x = "Type of Disposition",
...
```{r}
#create same barchart but with descending bars
all_dispo %>% 
  ggplot(aes(x=reorder(disposition, n), y=n, fill=n, weight=n)) +
  coord_flip()+
  theme_minimal() +
  geom_col(position = "dodge")+
    labs(
