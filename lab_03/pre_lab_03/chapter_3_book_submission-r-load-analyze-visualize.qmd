...
Editors %>%
  arrange(desc(Salary))
#highest paid editor is a Senior Photo Editor at Tribune Publishing who earns $770,000
...
#Executive Editor, News. Black, gender is noanswer. Salary is $300,000
MediaBucks %>%
  filter(COMPANY == "NewYorkTimes") %>%
  arrange(desc(Salary))
#the highest paid position at NYT is an Executive Editor who is Black and who put no answer to gender 
```
```{r}
#alternative way to get question #2: 
NYT %>%
  select(Gender, Salary) %>%
  group_by(Gender) %>%
  summarize(Salary = max(Salary, na.rm=TRUE))
NYT %>%
  select(Race, Salary) %>%
  group_by(Race) %>%
  summarize(Salary = max(Salary, na.rm=TRUE))
...
#compare to NYT results
52000 - 50350
#NYT median salary is $1650 more than the WSJ median salary
```
