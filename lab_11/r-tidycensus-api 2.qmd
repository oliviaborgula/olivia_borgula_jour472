...
```{r}
#find which US state has the largest male population
...
#California - 19,562,882 people

#find what state has a highest proportion of males 
male_pct <- male %>%
  inner_join(total_pop_2020, by ="state")%>%
  mutate(pct_state = percent(estimate / total_population))%>%
  arrange(desc(pct_state))

#Alaska has the highest percentage of males in its population, with 52.45 percent of the population being male
