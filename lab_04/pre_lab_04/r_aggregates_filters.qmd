...
baltcity_income<- read_csv('/Users/oliviaborgula/Documents/GitHub/olivia_borgula_jour472/lab_04/pre_lab_04/baltcity_income_clean.csv') %>% 
  as.data.frame()
...
baltcity_race <- read_csv('/Users/oliviaborgula/Documents/GitHub/olivia_borgula_jour472/lab_04/pre_lab_04/baltcity_race_8_13.csv') %>% 
  as.data.frame()
```
...
```{r}
#median income for Baltimore City: $49,875
#construct a filter for all census tracts below the citywide meidan household income for 2020 ($49,875)
baltcity_income %>%
  select(Neighborhood, x2020) %>% 
  filter(x2020 < 49875) %>% 
  arrange(desc(x2020))
#count number below the median
baltcity_income %>%
  group_by(Neighborhood) %>%
  select(Neighborhood, x2020) %>%
  filter(x2020 < 49875) %>%
...
```{r}
#median income for Baltimore City: $49,875
#table showing all Baltimore census tracts that are below the citywide median household income in 2020
below_median_tracts <- baltcity_income %>%
  select(Neighborhood, x2020) %>% 
  filter(x2020 < 49875) %>% 
  arrange(desc(x2020))

#count of Baltimore census tracts below the median household income in 2020
baltcity_income %>%
  select(Neighborhood, x2020) %>%
  filter(x2020 < 49875) %>%
  summarise(
...
#what percent of Bmore census tracts are below the citywide median?
#count total tracts in Baltimore
baltcity_income %>%
   summarise(
    count_tracts = n()
  )
#divide # of census tracts below median household income in 2020 by total census tracts in 2020 to get what percent of Bmore is below med. household income
(97/200) * 100
#48.5% of Baltimore City census tracts have a median household income below $49,875

#Tweet
#Nearly 50% of Baltimore City neighborhoods are below the citywide median household income of $49,875. 
```
